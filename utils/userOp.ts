...
import { EcdsaOwnershipRegistryModule__factory, EntryPoint, SmartAccountFactory__factory } from "../typechain-types";
...
export async function fillUserOp(
  ...
  try {
    op1.maxFeePerGas = feeData.maxFeePerGas!.add(1e8);
    op1.maxPriorityFeePerGas = feeData.maxPriorityFeePerGas!.add(5e8);
  } catch (err) {
    throw new Error("Network Error: Fee data for the chain is not found");
  }
...
export function ecdsaOwnershipSetupData(accountOwner: address) {
  return EcdsaOwnershipRegistryModule__factory.createInterface().encodeFunctionData(
    "initForSmartAccount",
    [accountOwner]
  );
}
...
export function getInitCode(
  ...
  return ethers.utils.hexConcat(
    [
      factory,
      SmartAccountFactory__factory.createInterface().encodeFunctionData("deployCounterFactualAccount", [
        sessionKeyModule,
        ecdsaModule,
        ecdsaOwnershipSetupData(accountOwner),
        index
      ])
    ]
  )
